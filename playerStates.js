...
export class Sitting extends State {
    ...
    handleInput(input){
        if(input.includes('ArrowLeft') || input.includes('ArrowRight')){
            this.game.player.setState(states.RUNNING, 6);
        }else if(input.includes('Enter')){
            this.game.player.setState(states.ROLLING, 15);
        }
    }
}
...
export class Running extends State {
    ...
    handleInput(input){
        ...
        if (input.includes('ArrowDown') && input.length === 1){
            this.game.player.setState(states.SITTING, 0);
        } else if (input.includes('ArrowUp')){
            this.game.player.setState(states.JUMPING, 8);
...
export class Jumping extends State {
    ...
    handleInput(input){
        if(this.game.player.vy > this.game.player.weight){
            this.game.player.setState(states.FALLING, 8);
        } else if (input.includes('Enter')) {
            this.game.player.setState(states.ROLLING, 15);
